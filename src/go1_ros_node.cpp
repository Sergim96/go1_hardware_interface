...
int main(int argc, char **argv) {
  ...
  ros::AsyncSpinner spinner(1);  // Argument is the number of threads to use (0
                                 // means as many threads as processors)
  spinner.start();
  ...
  while (ros::ok()) {
    ...
    if (elapsed_seconds >= period)
      ...
      std::this_thread::sleep_for(
          std::chrono::duration<double>(period - elapsed_seconds));  // Sleep the remaining time
